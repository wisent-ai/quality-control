...
Every guard's command line, flags, JSON report shape and refusals, the fleet
audit, the informative-commit action and the reusable workflows with their
consumers are in [docs/interfaces.md](docs/interfaces.md). In short: a source
guard takes one mode (`--all`, `--staged`, `--worktree`, `--base <sha>`,
`--range <before>..<after>`; `--staged` when none is given), `--json` prints
one report object, and exit `0` is clean, `1` findings, `2` a refused call.
